...
def check_for_lateness(row):
    if option == "Today":
        cutoff_time = datetime.datetime.strptime(f"{datetime.today().strftime('%Y-%m-%d')} {row['cutoff']}", "%Y-%m-%d %H:%M") 
    elif option == "Yesterday":
        cutoff_time = datetime.datetime.strptime(f"{datetime.today().strftime('%Y-%m-%d') - datetime.timedelta(days=1)} {row['cutoff']}", "%Y-%m-%d %H:%M")
